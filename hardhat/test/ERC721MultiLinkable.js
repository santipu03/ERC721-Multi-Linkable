...
describe("E7ML Basic Implementation", function () {
  let E7MLBasic, token1, token2, account1, account2;

  async function deploy() {
    const deployed = await deployBasic();
    E7MLBasic = deployed.E7MLBasic;
    token1 = deployed.token1;
    token2 = deployed.token2;
    account1 = deployed.account1;
    account2 = deployed.account2;

    // Mint E7ML tokens to account1 and account2
    await E7MLBasic.mint(0);
    await E7MLBasic.connect(account2).mint(1);
  ...
  describe("Deployment", function () {
    it("should check ownership of token 0", async function () {
      expect(await E7MLBasic.ownerOf(0)).to.equal(account1.address);
    });
  });

  describe("linkToken()", function () {
    it("should not be linked", async function () {
      const res = await E7MLBasic.tokenInfo(0);
    ...
    it("should link token", async function () {
      await E7MLBasic.linkToken(0, token1.address, 1);
      const res = await E7MLBasic.tokenInfo(0);
    ...
    it("should emit Link event", async function () {
      expect(await E7MLBasic.linkToken(0, token1.address, 1))
        .to.emit(E7MLBasic, "Link")
        .withArgs(0, token1.address, 1);
    });

    it("should revert with invalid token ID", async function () {
      await expect(
        E7MLBasic.linkToken(0, token1.address, 2)
    ...
    it("Should revert with token already linked", async function () {
      await E7MLBasic.linkToken(0, token1.address, 1);
      await expect(
        E7MLBasic.linkToken(0, token1.address, 1)
      ).to.be.revertedWith("ERC721MultiLinkable: token is already linked");
    });

    it("Should revert with caller is not owner nor aproved of E7ML token", async function () {
      await expect(
        E7MLBasic.linkToken(1, token2.address, 1)
      ).to.be.revertedWith("ERC721: caller is not token owner nor approved");
    });

    it("Should revert with caller is not owner of ParentToken", async function () {
      await expect(
        E7MLBasic.linkToken(0, token2.address, 1)
      ).to.be.revertedWith("ERC721MultiLinkable: token owners do not match");
    });
  });

  describe("syncToken()", function () {
    it("should NOT transfer E7ML token when transferring parentToken", async function () {
      await E7MLBasic.linkToken(0, token1.address, 1);
      await token1.transferFrom(account1.address, account2.address, 1);
      expect(await token1.ownerOf(1)).to.be.equal(account2.address);
      expect(await E7MLBasic.ownerOf(0)).to.be.equal(account1.address);
    });

    it("should transfer E7ML token when when transferring parentToken AND calling syncToken", async function () {
      await E7MLBasic.linkToken(0, token1.address, 1);
      await token1.transferFrom(account1.address, account2.address, 1);
      await E7MLBasic.syncToken(0);
      expect(await E7MLBasic.ownerOf(0)).to.be.equal(account2.address);
    });

    it("should revert if token is already synced", async function () {
      await E7MLBasic.linkToken(0, token1.address, 1);
      await expect(E7MLBasic.syncToken(0)).to.be.revertedWith(
  ...
  describe("_beforeTokenTransfer()", function () {
    it("Should revert with is not linked", async function () {
      await expect(
        E7MLBasic.transferFrom(account1.address, account2.address, 0)
      ).to.be.revertedWith(
  ...
  describe("tokenInfo()", function () {
    it("should revert with invalid token ID", async function () {
      await expect(E7MLBasic.tokenInfo(3)).to.be.revertedWith(
